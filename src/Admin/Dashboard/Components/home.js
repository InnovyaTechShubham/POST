...
function Home() {
  ...
  const gethospital = async () => {
    try {
      ...
      const { data } = await Axios.get(`${BASE_URL}/hospitals`); // Updated URL
  ...
  const getstock = async () => {
    try {
      const BASE_URL = process.env.BASE_URL || "http://localhost:4000"; // Default to localhost if not provided in env
      const { data } = await Axios.get(`${BASE_URL}/stocks`); // Updated URL
...
const stockcount = uniqueValues1.size;
  ...
  const getissued = async () => {
    try {
      const BASE_URL = process.env.BASE_URL || "http://localhost:4000"; // Default to localhost if not provided in env
      const { data } = await Axios.get(`${BASE_URL}/issueds`); // Updated URL
  ...
  const gethistory = async () => {
    try {
      const BASE_URL = process.env.BASE_URL || "http://localhost:4000"; // Default to localhost if not provided in env
      const { data } = await Axios.get(`${BASE_URL}/history`); // Updated URL
  ...
  const getprodnew = async () => {
    try {
      const BASE_URL = process.env.BASE_URL || "http://localhost:4000"; // Default to localhost if not provided in env
      const { data } = await Axios.get(`${BASE_URL}/products`); // Updated URL
